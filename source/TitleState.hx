...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('red/credits/bg','shared'));
		// bg.antialiasing = true;
		// bg.setGraphicSize(Std.int(bg.width * 0.6));
		// bg.updateHitbox();
		add(bg);

		logoBl = new FlxSprite(250, -100);
		logoBl.frames = Paths.getSparrowAtlas('red/sussylogo','shared');
		...
		swagShader = new ColorSwap();
			...
			gfDance.frames = Paths.getSparrowAtlas('red/red dance','shared');
			gfDance.animation.addByPrefix('idle', 'red dance0', 60);
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			// credTextShit.text += '\nNight';
			case 15:
				addMoreText('Red'); // credTextShit.text += '\nFunkin';
