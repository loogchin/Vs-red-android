...
class PlayState extends MusicBeatState
{
	...
	var camMovement:Float = 0.07;
	public var camZoom:Float;
	var why:FlxText;

	//red bg
	var darkskybg:BGSprite;
	var deadtree:BGSprite;
	var door:BGSprite;
	var dirt:BGSprite;

	public var youissus:Bool = false;

	//end song
	var bgchar:BGSprite;

	//change bg
	var wireddarkskybg:BGSprite;
	var wireddoor:BGSprite;
	var wireddirt:BGSprite;
	...
	var hotdog:BGSprite;
	var nightblack:BGSprite;

	var redeffect:BGSprite;

	var blacksus:BGSprite;
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['oh wtf how', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	{
		youissus = (SONG.song.toLowerCase() == 'redsus');
		...
		switch (curStage)
		{
			...
			case 'redbgvibe':
						...
						darkskybg = new BGSprite('red/vibe/skybg1', -553, -150, 0.9, 0.9);
						darkskybg.antialiasing = true;
						darkskybg.active = false;
						add(darkskybg);

						darkskybg = new BGSprite('red/vibe/deadtree1', -553, -150, 0.9, 0.9);
						deadtree.antialiasing = true;
						deadtree.active = false;
						add(deadtree);

						door = new BGSprite('red/vibe/door1', -553, -150, 0.9, 0.9);
						door.antialiasing = true;
						door.active = false;
						add(door);

						dirt = new BGSprite('red/vibe/dirt1', -553, -150, 0.9, 0.9);
						dirt.antialiasing = true;
						dirt.active = false;
						add(dirt);
			case 'redbgvibe2':
						curStage = 'redbg';
						
						darkskybg = new BGSprite('red/vibe/skybg2', -553, -150, 0.9, 0.9);
						darkskybg.antialiasing = true;
						darkskybg.active = false;
						add(darkskybg);

						deadtree = new BGSprite('red/vibe/deadtree2', -553, -150, 0.9, 0.9);
						deadtree.antialiasing = true;
						deadtree.active = false;
						add(deadtree);

						door = new BGSprite('red/vibe/door2', -553, -150, 0.9, 0.9);
						door.antialiasing = true;
						door.active = false;
						add(door);

						dirt = new BGSprite('red/vibe/dirt2', -553, -150, 0.9, 0.9);
						dirt.antialiasing = true;
						dirt.active = false;
						add(dirt);
			case 'redbg':
						...
						darkskybg = new BGSprite('red/skybg', -553, -150, 0.9, 0.9);
						darkskybg.antialiasing = true;
						darkskybg.active = false;
						add(darkskybg);

						deadtree = new BGSprite('red/deadtree', -553, -150, 0.9, 0.9);
						deadtree.antialiasing = true;
						deadtree.active = false;
						add(deadtree);

						door = new BGSprite('red/door', -553, -150, 0.9, 0.9);
						door.antialiasing = true;
						door.active = false;
						add(door);

						dirt = new BGSprite('red/dirt', -553, -150, 0.9, 0.9);
						dirt.antialiasing = true;
						dirt.active = false;
						add(dirt);

						if (PlayState.SONG.song.toLowerCase() == "end"){
							bgchar = new BGSprite('red/bg character', -640, 260, 0.9, 0.9, ['bg character']);
							bgchar.animation.addByPrefix('idle', 'bg character', 24, false);
							add(bgchar);
						}
			...
			case 'shadowredbg':
						...
						darkskybg = new BGSprite('red/redskybg', -553, -150, 0.9, 0.9);
						darkskybg.antialiasing = true;
						darkskybg.active = false;
						add(darkskybg);

						door = new BGSprite('red/reddoor', -553, -150, 0.9, 0.9);
						door.antialiasing = true;
						door.active = false;
						add(door);

						dirt = new BGSprite('red/reddirt', -553, -150, 0.9, 0.9);
						dirt.antialiasing = true;
						dirt.active = false;
						add(dirt);

						//change bg bruh :)
						wireddarkskybg = new BGSprite('red/wired red bg', -553, -150, 0.9, 0.9);
						wireddarkskybg.antialiasing = true;
						wireddarkskybg.active = false;
						add(wireddarkskybg);

						wireddoor = new BGSprite('red/wired red door', -553, -150, 0.9, 0.9);
						wireddoor.antialiasing = true;
						wireddoor.active = false;
						add(wireddoor);

						wireddirt = new BGSprite('red/wired red', -553, -150, 0.9, 0.9);
						wireddirt.antialiasing = true;
						wireddirt.active = false;
						add(wireddirt);

						blacksus = new BGSprite('red/black', 0, 0, 0.9, 0.9);
						blacksus.antialiasing = true;
						blacksus.active = false;
						blacksus.cameras = [camHUD];
						add(blacksus);

						wireddarkskybg.visible = false;
						wireddoor.visible = false;
						wireddirt.visible = false;

						//boyfriend = new Boyfriend(0, 0, 'wire-bf');
						//gf = new Character(0, 0, 'wire-gf');
						//dad = new Character(0, 0, 'wire-shadow');
						//fix lag change character :)
						//bf Character :)
						//boyfriend = new Boyfriend(0, 0, 'your bf character');
						//dad Character :)
						//dad = new Character(0, 0, 'your dad character');
						//gf Character :)
						//gf = new Character(0, 0, 'your gf character'); 
			case 'sus':
						curStage = 'sus';
						tree = new BGSprite('red/sus/stageback', -600, -200, 0.9, 0.9);
						tree.antialiasing = true;
						tree.active = false;
						add(tree);
						
						nightblack = new BGSprite('red/sus/nightblack', -400, 34);
						nightblack.antialiasing = true;
						nightblack.scrollFactor.set(0.9, 0.9);
						nightblack.active = false;
						add(nightblack);

						hotdog = new BGSprite('red/sus/hotdog', 1300, 164);
						hotdog.antialiasing = true;
						hotdog.scrollFactor.set(0.9, 0.9);
						hotdog.active = false;
						add(hotdog);
						
						back = new BGSprite('red/sus/stageback2', -600, -200);
						back.antialiasing = true;
						back.scrollFactor.set(0.9, 0.9);
						back.active = false;
						add(back);

						fonts = new BGSprite('red/sus/stagefront', -600, -200);
						fonts.antialiasing = true;
						fonts.scrollFactor.set(0.9, 0.9);
						fonts.active = false;
						add(fonts);

						sus1 = new BGSprite('red/sus/sus1', -602, 250, 0.9, 0.9, ['sus1']);
						sus1.animation.addByPrefix('idle', 'sus1', 24, true);
						add(sus1);

						sus2 = new BGSprite('red/sus/sus2', -361, 482, 0.9, 0.9, ['sus1']);
						sus2.animation.addByPrefix('idle', 'sus1', 24, true);
						add(sus2);

						sus3 = new BGSprite('red/sus/sus3', 918, 482, 0.9, 0.9, ['sus1']);
						sus3.animation.addByPrefix('idle', 'sus1', 24, true);
						add(sus3);

						sus4 = new BGSprite('red/sus/sus4', 1259, 453, 0.9, 0.9, ['sus1']);
						sus4.animation.addByPrefix('idle', 'sus1', 24, true);
						add(sus4);

						/*redeffect = new FlxSprite(1001.05, 549.1);
						redeffect.frames = Paths.getSparrowAtlas('sus/kill','red');
						redeffect.animation.addByPrefix('idle', 'killing bg0', 30);
						redeffect.animation.play('idle');
						redeffect.scrollFactor.set(0.9, 0.9);
						redeffect.alpha = 0;
						add(redeffect);*/
		}
		...
		switch(curStage)
		{
			...
			case 'schoolEvil':
				...
				add(evilTrail);
			case 'shadowredbg':
				var sussyeffect = new FlxTrail(dad, null, 1, 24, 0.6, 0.9);
				add(sussyeffect);
			case 'redbg':
				if (PlayState.SONG.song.toLowerCase() == "end"){
					var sussyeffect = new FlxTrail(dad, null, 1, 24, 0.6, 0.9);
		...
		// Add why
		why = new FlxText("My brain is too small.");
		why.setFormat(Paths.font("vcr.ttf"), 18, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'redsus')
			switch (curStep)
			{
				case 1162:
					FlxTween.tween(camHUD, {alpha: 0}, {ease: FlxEase.expoInOut});
				case 1200:
					FlxTween.tween(camHUD, {alpha: 1}, {ease: FlxEase.expoInOut});
			}
	...
	function WireOn(id:Bool)
	{
		if(id)
		{
					FlxG.camera.flash(FlxColor.RED, 1);
					wireddarkskybg.visible = true;
					wireddoor.visible = true;
					wireddirt.visible = true;
					//changeBfCharacternoflip('wire-bf');
					//changeGfCharacter('wire-gf');
					//changeDadCharacter('wire-shadow');
					//remove(healthBarBG);
					//healthBarBG.loadGraphic(Paths.image('red/redhealthBar'));
					//add(healthBarBG);
					//healthBar.alpha = 0;
		}
		else
		{
					...
					wireddoor.visible = false;
					wireddirt.visible = false;
					//changeBfCharacternoflip('bf');
					//changeGfCharacter('gf');
					//changeDadCharacter('shadow');
					//remove(healthBarBG);
					//healthBarBG.loadGraphic(Paths.image('healthBar'));
					//add(healthBarBG);
					//healthBar.alpha = 1;
			//healthBar Color change is not working :(
			//healthBar.createFilledBar(FlxColor.fromRGB(54, 17, 36), FlxColor.fromRGB(49, 176, 209));
			//healthBar.createFilledBar(FlxColor.fromRGB(54, 17, 36), FlxColor.fromString('#FF' + boyfriend.playerColor));
		}
	...
	public function countdown():Void
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 200, function(tmr:FlxTimer)
				{
					if (tmr.loopsLeft % gfSpeed == 0)
					{
						gf.dance();
